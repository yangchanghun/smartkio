...
type Screen = "menu" | "kakao" | "signup" | "friend" | "chat" | "block" | "delivery";
...
export default function App() {
  ...
  if (screen === "signup") return <KakaoSignupScreen onBack={() => setScreen("kakao")} />;
  if (screen === "friend") return <FriendAddScreen onBack={() => setScreen("kakao")} />;
  if (screen === "chat") return <ConversationScreen onBack={() => setScreen("kakao")} />;
  if (screen === "block") return <BlockFriendScreen onBack={() => setScreen("kakao")} />;
  if (screen === "kakao") return <KakaoTutorialScreen onBack={() => setScreen("menu")} onSelect={setScreen} />;
  if (screen === "delivery") return <DeliveryPracticeScreen onBack={() => setScreen("menu")} token={session.token} />;
  return (
    <MenuScreen
      session={session}
      onLogout={logout}
